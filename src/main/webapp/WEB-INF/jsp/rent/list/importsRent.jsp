...
<input type="hidden" name="infoValue" id="infoValue" value="2"/>
...
<input type="hidden" name="searchCondition" id="search_carCompId" value="${param.carCompId}" />
	<div class="amazon-tab-container">
		<div class="amazon-tabs">
			...
			<a href="/rent/imports"><div class="amazon-tab active">수입신차</div></a>
	...
	<form target="_self" method="get" id="searchForm">
		...
		<div class="rent-filter-container">
			<table class="search-table">
				...
				<%-- <tr>
					<th colspan="2">악정주행거리</th>
					<td>
						<input type="radio" value="20000" name="agreeDist" checked id="dist-2"/>
						<label for="dist-2">20,000km</label>
					</td>
					<td>
						<input type="radio" value="30000" name="agreeDist" id="dist-3"/>
						<label for="dist-2">30,000km</label>
					</td>
					<td>
						<input type="radio" value="40000" name="agreeDist" id="dist-4"/>
						<label for="dist-2">40,000km</label>
					</td>
					<td></td>
					<td></td>
				</tr> --%>
	...
	<table class="rent-list tablesorter sticky-enabled main-list" id="myTable">
		...
		<thead>
			...
			<tr class="rent-list-sub-head" onmouseover="this.style.cursor='default'">
				...
				<th colspan="4">장기렌트 월대여료(원, VAT 별도)</th>
			</tr>
			<tr onmouseover="this.style.cursor='default'">
				...
				<th colspan="3" id="test">정비 미포함(기본식)&nbsp;&nbsp;
			...
			<c:forEach var="data" items="${list}" varStatus="status">
				<tr>
					...
					<td>
						...
						<input type="hidden" value="${data.AGREE_DIST}">
					</td>
					...
					<c:choose>
						<c:when test="${data.CAR_COMP_ID ne '0056'}">
							<td id="${data.RB60_ID}" class="list-price" style="color: #16519c !important;">
								<c:choose>
									<c:when test="${data.RB60_AMT eq 0}">미운영</c:when>
									<c:otherwise>
										<fmt:formatNumber value="${data.RB60_AMT}" pattern="#,###" />
									</c:otherwise>
								</c:choose>
								<%-- <fmt:formatNumber value="${data.RB60_AMT}" pattern="#,###" /> --%>
							</td>
							<td id="${data.RB48_ID}" class="list-price">
								<c:choose>
									<c:when test="${data.RB48_AMT eq 0}">미운영</c:when>
									<c:otherwise>
										<fmt:formatNumber value="${data.RB48_AMT}" pattern="#,###" />
									</c:otherwise>
								</c:choose>
								<%-- <fmt:formatNumber value="${data.RB48_AMT}" pattern="#,###" /> --%>
							</td>
							<td id="${data.RB36_ID2}" class="list-price">
								<c:choose>
									<c:when test="${data.RB36_AMT eq 0}">미운영</c:when>
									<c:otherwise>
										<fmt:formatNumber value="${data.RB36_AMT}" pattern="#,###" />
									</c:otherwise>
								</c:choose>
								<%-- <fmt:formatNumber value="${data.RB36_AMT}" pattern="#,###" /> --%>
							</td>
						...
						<c:when test="${data.CAR_COMP_ID eq '0056'}">
							<c:choose>
								<c:when test="${data.AGREE_DIST >= 40000}">
									<td id="${data.RB60_ID}" class="list-price" style="color: #16519c !important;">미운영</td>
									<td id="${data.RB48_ID}" class="list-price">미운영</td>
									<td id="${data.RB36_ID2}" class="list-price">미운영</td>
								</c:when>
								<c:otherwise>
									...
									<td id="${data.RB36_ID2}" class="list-price"><fmt:formatNumber value="${data.RB36_AMT}" pattern="#,###" /></td>
								</c:otherwise>
							</c:choose>
						</c:when>
						<c:otherwise>
	...
	<input type="hidden" id="urlForCookie" value="/rent/imports/detail" />
	<!--  hidden form -->
	<form id="detailFrm" action="/rent/imports/detail" method="get">
