...
$(document).ready(function(){
	...
	$('.detail-btn-print').click(function(){
		...
		//테슬라차량 20000으로 고정
		/* if (car_comp_id == "0056") {	
			url = "/lease/print?estimateId=" + id + "&seq=" + seq + "&refer=detail&agreeDist=20000";
		} else {
			url = "/lease/print?estimateId=" + id + "&seq=" + seq + "&refer=detail&agreeDist=" + agreeDist;
		} */
	...
	var type = href.split("/")[2]; //domestic 또는 imports 만 남는다
	$("a[href='"+href+"']").find("div").addClass("active");
	$('.detail-title a').attr("href","/lease/"+type);
	...
	$('.detail-price').click(function(){
		var id = $(this).attr("id");
		var url = "/lease/print?estimateId=" + id;
		window.open(url,"popup","width=800,height=800,top=0,left=100");
	})
...
<link href="https://cdnjs.cloudflare.com/ajax/libs/Swiper/3.3.1/css/swiper.min.css" rel="stylesheet"/>
  		   ...
  		   <div class="domestic-left-content">
		    	...
		    	<table>
			    	<tr>
			    		...
			    		<td class="car-info-value">${mainData.COM_NM}</td>
			    		<th class="car-info-title">배기량</th>
			    		<td class="car-info-value">${detailData.DPM}cc</td>			    		
			    	</tr>
			    	<tr>
			    	...
			    	<tr>
			    		...
			    		<th class="car-info-title">차량가격</th>
	...
	<div class="tab-content price-tab">
		<ul class="nav nav-tabs amazon-detail-tab" role="tablist">
			<li role="presentation" class="active"><a href="#leasePlus" aria-controls="leasePlus" role="tab" data-toggle="tab">리스</a></li>
	   		<li role="presentation"><a href="#rent" aria-controls="rent" role="tab" data-toggle="tab">장기렌트</a></li>
	 	</ul>
	    <div role="tabpanel" class="tab-pane active domestic-content-box price-box" id="leasePlus">
			    		...
			    		<tbody>
				    		<c:forEach var="priceData" items="${priceList}" varStatus="status">
						    	...
						    	<tr id="${priceData.AGREE_DIST}" class="price-info-tr">
						    		...
						    		<td><span class="text-bold"><fmt:formatNumber value="${priceData.LB60_AMT}" pattern="#,###" />원</span></td>
						    	...
						    	<tr id="${priceData.AGREE_DIST}" class="price-info-tr">
						    		...
						    		<td><span class="text-bold"><fmt:formatNumber value="${priceData.LB48_AMT}" pattern="#,###" />원</span></td>
						    	...
						    	<tr id="${priceData.AGREE_DIST}" class="price-info-tr">
						    		...
						    		<td><span class="text-bold"><fmt:formatNumber value="${priceData.LB36_AMT}" pattern="#,###" />원</span></td>
			    		...
			    		<tbody>
					    	<c:forEach var="priceData" items="${priceList}" varStatus="status">    	
						    	...
						    	<tr id="${priceData.AGREE_DIST}" class="price-info-tr">
						    		<td class="price-unit">60개월</td>
						    		<c:choose>
				    					<c:when test="${mainData.CAR_KIND eq '화물' && priceData.LS60_AMT eq -1}">
				    						<td colspan="3">미운영</td>
				    					</c:when>
				    					<c:otherwise>
				    						<td><span class="text-bold"><fmt:formatNumber value="${priceData.LS60_AMT}" pattern="#,###" />원</span></td>
						    	...
						    	<tr id="${priceData.AGREE_DIST}" class="price-info-tr">
						    		...
						    		<c:choose>
				    					...
				    					<c:otherwise>
				    						<td><span class="text-bold"><fmt:formatNumber value="${priceData.LS48_AMT}" pattern="#,###" />원</span></td>
						    	...
						    	<tr id="${priceData.AGREE_DIST}" class="price-info-tr">
						    		...
						    		<c:choose>
				    					...
				    					<c:otherwise>
				    						<td><span class="text-bold"><fmt:formatNumber value="${priceData.LS36_AMT}" pattern="#,###" />원</span></td>
	    ...
	    <div role="tabpanel" class="tab-pane domestic-content-box price-box" id="rent">
	    	<div class="price-tab-left">
			    	...
			    	<table>
			    		...
			    		<tbody>
				    		<c:forEach var="priceData" items="${priceList}" varStatus="status">
						    	...
						    	<tr id="${priceData.AGREE_DIST}" class="price-info-tr">
						    		<td class="price-unit">60개월</td>
						    		<td><span class="text-bold"><fmt:formatNumber value="${priceData.RB60_AMT}" pattern="#,###" />원</span></td>
						    	...
						    	<tr id="${priceData.AGREE_DIST}" class="price-info-tr">
						    		<td class="price-unit">48개월</td>
						    		<td><span class="text-bold"><fmt:formatNumber value="${priceData.RB48_AMT}" pattern="#,###" />원</span></td>
						    	...
						    	<tr id="${priceData.AGREE_DIST}" class="price-info-tr">
						    		<td class="price-unit">36개월</td>
						    		<td><span class="text-bold"><fmt:formatNumber value="${priceData.RB36_AMT}" pattern="#,###" />원</span></td>
			    	...
			    	<table>
			    		...
			    		<tbody>		
					    	<c:forEach var="priceData" items="${priceList}" varStatus="status">   
						    	...
						    	<tr id="${priceData.AGREE_DIST}" class="price-info-tr">
						    		<td class="price-unit">60개월</td>
					    			<td class="detail-price"><span class="text-bold"><fmt:formatNumber value="${priceData.RS60_AMT}" pattern="#,###" />원</span></td>
	...
	<c:choose>
		<c:when test="${(mainData.JG_G_7 eq 3 || mainData.JG_G_7 eq 4) && (mainData.CAR_COMP_ID ne '0056') && !(detailData.JG_CODE eq '9133' || detailData.JG_CODE eq '9015435')}">
			...
			<div style=""></div>
				보증금
				<%-- <c:choose>
					<c:when test="${mainData.JG_G_7 eq 3}">
						<c:if test="${estimateData.CAR_AMT >= 45000000}">15%</c:if><!-- 25%-10% -->
						<c:if test="${estimateData.CAR_AMT < 45000000}">10%</c:if><!-- 20%-10% -->
					</c:when>	 
					<c:otherwise>
						10% <!-- 수소차(나중엔 수식이 바뀔수 있음.현재는 이렇게 하기로(20190218))-->
					</c:otherwise>
				</c:choose> --%>
				<c:choose>
