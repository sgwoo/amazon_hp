...
#periodSelectBox option:disabled {
	color: #D5D7DD;
}
#agreeDistSelectBox option:disabled {
	color: #D5D7DD;
}
...
//	}
	...
	$('#detail-modal').on('shown.bs.modal', function (e) {
		...
		$('td.title').click(function(){ //차종 명 선택 시
			...
			/* var jg_tuix_st = $(this).next().next().attr("id").split("/")[0];
			var lkas_yn = $(this).next().next().attr("id").split("/")[1];
			var ldws_yn = $(this).next().next().attr("id").split("/")[2];
			var aeb_yn = $(this).next().next().attr("id").split("/")[3];
			var fcw_yn = $(this).next().next().attr("id").split("/")[4];
			var hook_yn = $(this).next().next().attr("id").split("/")[5]; */
			var jg_tuix_st = $(this).parent().find("td:last").attr("id").split("/")[0];
			var lkas_yn = $(this).parent().find("td:last").attr("id").split("/")[1];
			var ldws_yn = $(this).parent().find("td:last").attr("id").split("/")[2];
			var aeb_yn = $(this).parent().find("td:last").attr("id").split("/")[3];
			var fcw_yn = $(this).parent().find("td:last").attr("id").split("/")[4];
			var hook_yn = $(this).parent().find("td:last").attr("id").split("/")[5];
						
			if (jg_tuix_st == "undefined" || jg_tuix_st == "") {
				$("#jg_tuix_st").val("");
			} else {
				$("#jg_tuix_st").val(jg_tuix_st);				
			}
			
			if (lkas_yn == "undefined" || lkas_yn == "") {
				$("#lkas_yn").val("");
			} else{
				$("#lkas_yn").val(lkas_yn);				
			}
			
			if (ldws_yn == "undefined" || ldws_yn == "") {
				$("#ldws_yn").val("");
			} else{
				$("#ldws_yn").val(ldws_yn);				
			}
			
			if (aeb_yn == "undefined" || aeb_yn == "") {
				$("#aeb_yn").val("");
			} else{
				$("#aeb_yn").val(aeb_yn);				
			}
			
			if (fcw_yn == "undefined" || fcw_yn == "") {
				$("#fcw_yn").val("");
			} else{
				$("#fcw_yn").val(fcw_yn);				
			}
			
			if (hook_yn == "undefined" || hook_yn == "") {
				$("#hook_yn").val("");
			} else{
				$("#hook_yn").val(hook_yn);				
			}
	...
	$('.calculate-btn').click(function(){
		...
		// 테슬라 차량 약정 운행거리 10,000~30,000km 이내만 가능
		/* if($('#compSelectBox').val() == "0056"){
			var agreeDistValue = $('#agreeDistSelectBox').val();
			var directInputAgreeDist = $('#directInputAgreeDist').val().replace('km', '').replace(',', '');
			if(agreeDistValue < 10000 || agreeDistValue > 30000 ){
				alert('테슬라 차량은 약정운행거리 1~3만km만 가능합니다.');
				return false;
			} else if(agreeDistValue == 'directInput'){
				if(directInputAgreeDist < 10000 || directInputAgreeDist > 30000){
					alert('테슬라 차량은 약정운행거리 1~3만km만 가능합니다.');
					return false;
				}
			}
		...
		if (ecarFlag == '4' && period < 24) {//수소차는 대여기간이 24개월 이상일 시에만 견적내기 가능			
	...
	<form id="calculateFrm" method="post" action="/calculate/smart">
		...
		<input type="hidden" value="0" name="optAmt_m" id="optAmt_m" /><!-- 옵션잔가미반영금액 -->
		<input type="hidden" value="" name="optionGarnishYn" id="optionGarnishYn" /><!-- 옵션가니쉬색상Y/N -->
		...
		<input type="hidden" value="" name="dcSeq" id="dcSeq" /> 
		<input type="hidden" value="0" name="dcAmt" id="dcAmt" /> 
		<input type="hidden" value="" name="esti_d_etc" id="esti_d_etc" /> 
		<input type="hidden" value="0" name="ecarLoc" id="ecarLoc" /><!-- 전기차 고객주소지 -->
		<input type="hidden" value="0" name="hcarLoc" id="hcarLoc" /><!-- 수소차 고객주소지 -->
		...
		<!-- 20190701 -->
		<input type="hidden" value="" name="lkas_yn" id="lkas_yn" />
		<input type="hidden" value="" name="lkas_yn_opt_st" id="lkas_yn_opt_st" />
		<input type="hidden" value="" name="ldws_yn" id="ldws_yn" />
		<input type="hidden" value="" name="ldws_yn_opt_st" id="ldws_yn_opt_st" />
		<input type="hidden" value="" name="aeb_yn" id="aeb_yn" />
		<input type="hidden" value="" name="aeb_yn_opt_st" id="aeb_yn_opt_st" />
		<input type="hidden" value="" name="fcw_yn" id="fcw_yn" />
		<input type="hidden" value="" name="fcw_yn_opt_st" id="fcw_yn_opt_st" />
		<input type="hidden" value="" name="hook_yn" id="hook_yn" />
		<input type="hidden" value="" name="hook_yn_opt_st" id="hook_yn_opt_st" />
		...
		<input type="hidden" value="" name="duty_free_opt" id="duty_free_opt">
		<input type="hidden" value="" name="jg_b" id="jg_b">
		<input type="hidden" value="" name="new_license_plate" id="new_license_plate">
	</form>
	...
	<input type="hidden" id="ecar_10_yn" value="">
	...
	<div class="estimate-header" style="padding: 0px 40px 0px 40px !important; width: 100% !important;">
		<div class="process-tab" style="padding-top: 38px !important; width: 100% !important; padding-left: 0px !important; height: 100% !important;">
			<div class="col-md-12 col-xs-12">
				<h2 class="primary" style="margin: 0px !important; font-size: 1.5em !important;">실시간 견적내기 및 상담 요청</h2>
			</div>
			<div class="col-md-12 col-xs-12" style="padding: 45px 0px 0px 0px !important;">
				<div class="col-md-4 col-xs-4 process-item done" style="width: 33.33333% !important;">STEP 1:실시간 견적</div>
				<div class="col-md-4 col-xs-4 process-item" style="width: 33.33333% !important;">STEP 2:상담 요청</div>
				<div class="col-md-4 col-xs-4 process-item" style="width: 33.33333% !important;">STEP 3:상담 정보 확인</div>
			</div>
		</div>
	</div>
	<div class="estimate-body" style="width: 100% !important; padding: 0px 40px 0px 40px !important;">
		<div class="car-info">
			<table class="smart-info-table" style="width: 100% !important;">
				<colgroup>
					<col width="15%" />
					<col width="55%" />
					<col width="15%" />
				...
				<tbody>
					...
					<tr>
						<td>제조사</td>
						<td>
							<select id="countrySelectBox" style="width: 35%;">
								<option value="1">국산</option>
								<option value="2">수입</option>
							</select> 
							<select id="compSelectBox" style="width: 60%; float: right;">
					...
					<tr>
						<td>차명</td>
						<td colspan="3">
							<select class="carNameSelectBox" style="width: 35%;">
								<option value="0" disable>제조사를 먼저 선택하세요</option>
							</select>
							<button type="button" class="btn btn-default-reverse modal-btn"	data-toggle="modal" data-target="#detail-modal"	style="margin-left: 15px;">차종 선택</button> 
							<input type="text" disabled	style="width: 40%; height: 33px; margin-left: 14px;" class="selectedCarDetail" />
						</td>
						<%-- <td colspan="2" style="padding-left: 0px;">
							...
							<input type="text" disabled	style="width: 65%; height: 33px; float: left; margin-left: 14px;" class="selectedCarDetail" />
						</td> --%>
					</tr>
					<tr class="carInfo-row">
						<td>선택차량</td>
						<td>
							<span class="primary bold" id="displayCarName" style="font-size: 17px;"></span>
						</td>
						<td class="text-right bold">차량가격</td>
						<td class="text-right">
							<span class="carAmt bold">0원</span>
						</td>
					</tr>
					<!-- 옵션 테이블은 동적 생성한다 (esti-smart.js > setCarOption() 참고)-->
					<tr id="color-tr">
						<td class="border_b_gray" id="colorTitleRow">색상</td>
						<td class="border_b_gray" id="colorText" style="padding-left: 18px !important; padding-right: 0px !important;"></td>
						<td class="text-right bold border_b_gray">색상가격</td>
						<td class="text-right bold border_b_gray" style="padding-left: 0px !important;">
					...
					<tr id="color-jg-tr" style="display: none;">
						<td></td>
						<td colspan="3">
							<span id="color-jg-name"></span>
							<span id="color-jg-info" style="display: none;"></span>
					...
					<tr class="dcAmt-row" style="display: none;">
						...
						<td colspan="3" class="text-right">
							-<span id="displayDcAmt" class="bold">0원</span>
							<div id="displayEstiDcEtc"></div>
						</td>
					</tr>
					<tr class="tax-row" style="display: none;">
						<td id="displayTaxDcTitle">개별소비세 감면액</td>
						<td id="dispayTaxDcText"></td>
						<td></td>
						<td class="text-right">
							<span id="displayTaxDcAmtMinus">-</span><span id="displayTaxDcAmt" class="bold">0원</span>
						</td>
					</tr>
					<tr class="">
						<td colspan="2" class="border_b_white"></td>
						<td class="text-right bold border_b_white" style="font-size: 17px; vertical-align: bottom;">총 차량가격</td>
						<td class="text-right border_b_white" style="padding-left: 0px; vertical-align: bottom;">
							<span id="totalDisplayAmt" class="bold" style="color: #f00; font-size: 17px;">0원</span>
						</td>
						<%-- <td style="padding: 0px 0px 0px 12px;">
							<span id="totalDisplayAmt-info"> ( 
								<span>제조사 D/C</span> <span>&nbsp;및&nbsp;</span><span>개별소비세 감면액</span> 반영 후 가격 
							)</span>
						</td> --%>
					</tr>
					<tr class="last-row">						
						<td colspan="4" class="text-right" style="padding-left: 0px; vertical-align: top;">
							<div id="totalDisplayAmt2" style="font-weight: 400; padding-top: 10px; padding-bottom: 10px;">
								(차량가격  0원 + 옵션가격  0원 + 색상가격 0원)
							</div>
							<!-- 아래 내용은 위 포함된 내용으로 대체됨 20200323 -->
							<div id="totalDisplayAmt-info" style="padding-top: 10px; padding-bottom: 10px; display: none;">
								(<span>제조사 D/C</span><span>&nbsp;및&nbsp;</span><span>개별소비세 감면액</span> 반영 후 가격)
							</div>
						</td>
					</tr>
			...
			<table class="car-option-table" style="width: 100% !important;">
				<colgroup>
					<col width="17%" />
					<col width="33%" />
					<col width="17%" />
				...
				<tbody>
					...
					<tr>
						<td>대여상품</td>
						<td>
							<select id="gubunSelectBox" onchange="javascript:set_max_deposit_amt();">
					...
					<tr>
						...
						<td colspan="2">
							<span id="period-info" class="warning"></span>
						</td>
					</tr>
					<tr>
						<td>연간 약정운행거리</td>
						<td>
							...
							<input type="text" name="agreeDistSelectBox" id="directInputAgreeDist" value="" style="width: 48%; float: right;" disabled>
						</td>
						<td colspan="2">
							<div style='display: flex;'>
								...
								<div style="cursor: pointer; margin-left: 15px;">
						      		<div style="border: 2px solid #f98460; background-color: #f98460; height: 30px; width: 210px; text-align: center; display: flex; justify-content: center; align-items: center;">
					...
					<tr>
						...
						<td>
							...
							<input type="text" id="depositAmt" value="" disabled style="width: 63%; margin-left: 13px; float: right;" class="percent-select-box" onblur="javascript:percent_select_box();" />
						</td>
						<td colspan="2">
							<div>
								※보증금은 계약기간 만료 후 환불해 드립니다.<small>[보증금 100만원을 증액하면, 월대여료 5,500원(VAT포함)이 인하됩니다.(년리6.6%효과)]</small>
							</div>
						</td>
					</tr>
					<tr>
						...
						<td>
							...
							<input type="text" id="preAmt" value="0원" disabled style="width: 63%; margin-left: 13px; float: right;" class="percent-select-box" />
						</td>
						<td colspan="2">
					...
					<tr>
						...
						<td colspan="2">
							...
							<div style="margin-top: -55px;">
								<!-- ※ 고객님의 신용도에 따라 최소 초기납입금(보증금 및 선납금) 규정이 있으며, 초기납입금은 아마존카와 협의를 통하여 보증보험 증권으로 대체 할 수 있습니다. 
								<small>&nbsp; 단, 보증보험료는 고객부담이고, 초기납입금을 보증보험 증권으로 대체시 초기납입금의 이자효과가 적용되지 않아 월대여료가 상승됩니다.</small> -->
								※ 고객님의 신용도에 따라 최소 초기납입금(보증금 및 선납금) 규정이 있습니다.
							</div>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<%-- <td colspan="2">
							<span class="foreign-info label label-warning" style="display: none;">
							<i class="glyphicon glyphicon-info-sign"></i>&nbsp;수입차의 경우,자차 면책금 최소 금액이 500,000원입니다</span>
						</td> --%>
					</tr>
					<tr>
						<td colspan="4" class="gubun-line">
							<hr />
						</td>
					</tr>
					<tr id="car_etc_item">
...
<!-- 							<label for="etc_sun">전면썬팅</label> -->
							<span id="sn_span">* 전면/측후면 썬팅 포함&nbsp;&nbsp;&nbsp;&nbsp;</span>
							<span id="bn_span1">* 블랙박스 포함</span>
							<span id="bn_span2">* 2채널 블랙박스 포함</span>
					...
					<tr class="ecar-area" style="display: none;">
						<td>고객 주소지</td>
						<td>
							<select id="ecarLocationSelectBox" onchange="javascript:autoChangeRegionSel('3');">
								...
								<option value="10">전남/전북(광주제외)</option>
								<option value="11">경기</option>
<!-- 								<option value="12">전지역(국고보조금限지원 견적)</option> -->
								<option value="13">전지역(보조금 없는 견적)</option>
							</select>
						</td>
						<td colspan="2"></td>
					</tr>
					<tr class="hcar-area" style="display: none;">
						<td>고객 주소지</td>
						<td>
							<select id="hcarLocationSelectBox" onchange="javascript:autoChangeRegionSel('4');">								                	  
		                        <option value="0" selected>서울/경기</option>		                        
		                        <option value="1">인천</option>		                        
		                        <option value="2">강원</option>		                        
		                        <option value="3">대전</option>		                        
		                        <option value="7">세종/충남/충북(대전제외)</option>		                        
					...
					<tr>
						...
						<td colspan="2">
							<!-- <span id="region_coment_ts2" style="display: none;">※ 테슬라 서울 강서서비스센터에서 운행관련 교육 후 인도</span> -->
						</td>
					</tr>
				</tbody>
			</table>
			<div class="text-right" style="width: 100%;">
				<button type="button" class="calculate-btn" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> 견적 진행 중">견적 내기</button>
			</div>
			<!-- 전기차/수소차 제외 모든차량들 -->
			<div id="view_price_other_car" style="display: none;">
				<table class="car-price-table" style="width: 100% !important;">
					<col width="15%" />
					<col width="32%" />
					<col width="18%" />
					<col width="35%" />
					<tbody>
						<tr class="header">
							...
							<th colspan="2" style="text-align: right !important;">
								<small class="bold minus-info" style="color: red; display: none; padding-right: 30px;">
									<br>&nbsp;
									※월 대여료는 음수값이 나오면 안됩니다.(보증금 및 선납금 확인 후 견적내기 필요)
								</small>
								<small class="bold minus-info-2" style="color: red; display: none; padding-right: 30px;">
									<br>&nbsp;
									※보증금을 과다하게 입력하였습니다. 보증금을 줄여주세요.
								</small>
							</th>
						</tr>
						<tr>
						...
						<tr>
							...
							<td style="height: 60px; vertical-align: top;">
								<div class="total-price" style="width: 100% !important;"></div>
							</td>
					...
					<tbody>
						...
						<tr>
							...
							<td rowspan="2" colspan="2">
								<div class="view_sellingOpt_on">
									<!-- 전기차일때만 나옴 -->
									<span>매입옵션 가격(인수가)</span> <span class="estimate-price" style="margin-left: 50%;" id="sellingPrice">0원</span> <br />
						...
						<tr>
							...
							<td rowspan="2" style="padding-top: 40px;">
								<span class="view_sellingOpt_on">매입옵션(인수가) 조정</span>
							</td>
							<td rowspan="2" style="vertical-align: bottom; padding-left: 0px !important;">
								<div class="view_sellingOpt_on">
									<select style="width: 32%;" id="buyOption">
										<option id="initValue"></option>
										<option value="1">보증금과같게</option>
										<option value="2">금액직접입력</option>
									</select> <input type="text" style="width: 38%;" id="buyText">
						...
						<tr>
							...
							<td style="height: 60px; vertical-align: top;">
								<div class="total-price1" style="width: 100% !important;"></div>
							</td>
					...
					<tbody>
						<tr class="header">
							...
							<th colspan="2">
								...
								<small class="bold minus-info2" style="color: red; display: none;"><br>&nbsp;
								※월 대여료는 음수값이 나오면 안됩니다.(보증금 및 선납금 확인 후 견적내기 필요)</small>
							</th>
						</tr>
						<tr>
						...
						<tr>
							<td class="option-title" style="height: 60px; vertical-align: top; padding-top: 10px;">월 대여료</td>
							<td style="height: 60px; vertical-align: top;">
								<div class="total-price2" style="width: 100% !important;"></div>
							</td>
							<td colspan="2" style="vertical-align: top;"></td>
						</tr>
					</tbody>
	...
	<div class="estimate-footer" style="width: 100% !important; padding: 10px 40px !important;">
		<div class="col-md-12 col-xs-12" style="padding: 0px;">
			<div class="col-md-6 col-xs-6" style="padding-left: 0px;">
				<button type="button" id="print_btn" class="print-btn" style="width: 100% !important;">견적서 보기</button>
			</div>
			<div class="col-md-6 col-xs-6" style="padding-right: 0px;">
				<button type="button" id="send_btn" class="submit-btn" style="width: 100% !important;">상담 요청하기</button>
			</div>
		</div>
		<div style="text-align: left; padding-top: 30px; float: right;">
			<span style="font-weight: bold; font-size: 1.0em; letter-spacing: 0.5px;">
				※ 고객님께서 내신 견적을 저장하여 다시 보고싶거나 상담요청한 내역을 확인하고 싶으면 홈페이지 우측 상단의 로그인 화면에서 간편하게
			</span><br>
			<span style="font-weight: bold; font-size: 1.0em; letter-spacing: 0.5px; padding-left: 18px;">
