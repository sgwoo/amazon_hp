...
function carAmtText(){
	...
	if((${today} >= 20181114 || ${data.RENT_DT} >= 20181114) && ${data.RENT_DT} < 20190701){
    	...
    	if(${jgData.JG_3}*100 > 0){
    		...
    		//하이브리드	
    			...
    			if(car_c_amt<=23001000){
    				...
    				fee_add_amt1 = priceCutting(fee_add_amt1, "R", 1000);
    			}	
   			//그외	
    		}else{
    			fee_add_amt1 = priceCutting(fee_pp_amt*0.014, "R", 1000);	//fms처럼 맞출려면 0.014곱해줘야함.
    		...
    		if(fee_add_amt1 > 0){
    			if (${mainData.JG_CODE}=='6134'||${mainData.JG_CODE}=='6135'||${mainData.JG_CODE}=='6136'||${mainData.JG_CODE}=='6137') {
	    			fee_add_text1 = '2019년7월1일 이후 신차가 출고되어 개별소비세가 환원(3.5%->5%)될 경우<br>월대여료가 '+numberWithCommas(fee_add_amt1)+'원(공급가) 인상됩니다.';    				
    			} else {
    				fee_add_amt1 = 0;
    			}
    	...
    	if(jg_g_7=='1' && jg_g_8=='1' && bk_128  > 0){
    		fee_add_amt2 = priceCutting(bk_128/${data.A_B}, "R", 1000);
...
function setRegLocSt(){
	...
	if(eco_e_tag=='1'){	//맑은서울스티커 발급선택
		if(jg_g_7==3 || ecarFlag==3 || jg_g_7==3){ //전기차
			...
			}else if(ecar_loc_st==1){	//강원
			}else if(ecar_loc_st==1){	//대전
		...
		<div class="print-info" style="margin-top: 60px;">
			<div>
				<table>
					<tr>
						<td width="520px">
							<h4 style="text-align: left;"><span class="title-num"></span>&nbsp;대여차량</h4>
						</td>
						<td>
				...
				<table class="print-table table table-condensed table-bordered">
					...
					<tr>
						...
						<td>&nbsp;
							<c:if test="${data.DC_AMT > 0}">제조사D/C</c:if>
							...
							<c:otherwise> --%>
								<c:choose>
									...
									<c:when test="${data.ECAR_LOC_ST eq '3'}"> <!-- 대전 -->
										<small>※ 고객 사업장 또는 주소지 대전 기준 견적. 고정형 충전기 선택시 정부지원금을 초과하는 금액은 고객부담</small> 
										<%-- <c:choose>
									...
									<c:when test="${data.ECAR_LOC_ST eq '4'}"> <!-- 광주,전남,전북 --> 
										<!-- <small>※ 고객 사업장 또는 주소지 광주/전남/전북 기준 견적. 고정형 충전기 선택시 정부지원금을 초과하는 금액은 고객부담</small> -->
										<small>※ 서울 구매보조금 기준 견적. 고정형 충전기 선택시 정부지원금을 초과하는 금액은 고객부담</small>
									</c:when>
				...
				<div style="float:right;">
					<small>
						<c:choose>
							<c:when test="${detailData.JG_CODE eq '1232'}"> ※ LPG/휘발유 겸용차</c:when>	
							<c:when test="${detailData.JG_CODE eq '1242'}"> ※ LPG/휘발유 겸용차</c:when>
							<c:otherwise>
								<c:choose>
			...
			<div>
				...
				<!-- 문구VV -->
				<small>※ ①보증금은 계약기간 만료 후 환불해 드립니다. [보증금 100만원을 증액하면, 월대여료 5,500원(VAT포함)이 인하됩니다. (년리 6.6% 효과)]</small><br>
			...
			<div>
				<c:if test="${detailData.JG_CODE > 6999 || detailData.JG_CODE < 2000}">			  
			...
			<div>
				...
				<ul>
					<li>등록, 자동차세 납부, 정기검사 등도 아마존카에서 처리(고객 비용 부담 없음)</li>
					<li>FMS(Fleet Management System)을 통해서 차량유지관리내역 제공(www.amazoncar.co.kr)</li>
					<li>대여기간 만료시에는 인수/반납 선택형의 경우 차량반납, 연장이용(할인요금 적용), 매입옵션 행사 중 선택 가능하고,<br>
