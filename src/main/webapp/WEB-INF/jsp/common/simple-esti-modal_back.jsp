...
$( document ).ready(function() {
	...
	if (navigator.userAgent.match(/iPhone/) || navigator.userAgent.match(/iPad/) || navigator.userAgent.match(/Android/)) {	
		...
		$('#simple-est-form').validate({
			...
			messages:{
				estName	:	{ 	required:"고객명을 입력 해주세요"			},
				estTel	:	{	required:"연락처를 입력 해주세요"			},
				estSido	:	{	required:"시/도를 선택 해주세요"				},
				estGugun:	{	required:"구/군을 선택 해주세요"				},
				estEmail  :	{	required:"이메일 주소를 입력 해주세요"	},
			...
			submitHandler:function(form){
				...
				if(scriptValidation($('#estName').val())==true && scriptValidation($('#estTel').val())==true &&
				   scriptValidation($('#estEmail').val())==true && scriptValidation($('#etc-textarea2').val())==true){
					vali_chk = true;
				}
				if(vali_chk==true){
					//지역 이름 셋팅
					...
					data["estName"] = $('#estName2').val().trim();
					data["estTel"] = $('#estTel2').val().trim();
					data["estEmail"] = $('#estEmail2').val().trim();
					...
					$.ajax({
						...
						success:function(response){
							result = response;
							if (result == "0") {
								alert("간편상담 등록 오류입니다. 관리자에게 문의해주세요.");
							} else {
								alert("간편상담신청이 등록되었습니다.");
							}
							$('#simple-esti-modal2').modal('hide');
						}
					});
				}
			}
		...
		$('#simple-est-form').validate({
			...
			submitHandler:function(form){
				...
				var vali_chk = false;
				if(scriptValidation($('#estName').val())==true && scriptValidation($('#estTel').val())==true &&
				   scriptValidation($('#estEmail').val())==true && scriptValidation($('#etc-textarea').val())==true){
					vali_chk = true;
				}
		     	...
		     	<div class="modal-body">
		     		<table class="full-width">
						...
						<tbody>
							...
							<tr>	
								<td><span style="vertical-align: middle;">*</span>연락처</td>
								<td><input type="text" name="estTel" id="estTel2" class="full-width form-control input-sm" /></td>
							</tr>
							<tr><td colspan="2"><br /></td></tr>
							<tr>
								<td><span style="vertical-align: middle;">*</span>지역</td>
