...
function setDefaultInfo(){
	...
	if(($("#ecarFlag").val() == "3" || $("#ecarFlag").val() == "4") && $("#carCompId").val() != "0056"){	
...
function calculateFinalAmt(){
	...
	if (car_comp_id == 56) {
		...
		//차량인도지역 안내문구
		$("#region_coment_ts").css("display", "none");
	}
...
function setCarOption(list){
	...
	var totalSum = "<tr class='option-item' style='height:45px;;border-bottom:1px solid #eeeeee !important'>" +
						...
						"<td class='text-right' colspan='2'><span class='option-sum bold'>(0원)</span></td>" +*/
						/*"<td></td>" +*/
					"</tr>";
	var content = "";	
	...
	$.each(list, function(index, item){
		...
		content += "</td>" +
						/*"<td colspan='2' class='option-price' param='"+item.AMT+"' style='padding-left: 0px !important;'>" +*/ 
...
function setCarCode(param){
	$.ajax({
		...
		success:function(response){			
			...
			if ((response.JG_G_7=="3") && response.CAR_COMP_ID != "0056") {
				$("#print_type").val("6");
			} else {
				$("#print_type").val("1");
			}
...
function insertFormData(){
	...
	if (ecarFlag == 3) { //전기차인 경우
		...
		else if(ecar_loc == "2"){	$("#a_h").val("10");	}	//강원은 대구등록
		else if(ecar_loc == "3"){	$("#a_h").val("10");	}	//대전은 대구등록
		else if(ecar_loc == "4"){ //$("#a_h").val("9");			//광주/전남/전북은 대구등록
									$("#a_h").val("10");		}	//지자체보조금 소진으로 서울보조금으로수정(2018.05.08)
		else if(ecar_loc == "5"){	$("#a_h").val("10");	}	//대구는 대구등록(2018.04.17)
		else if(ecar_loc == "6"){	$("#a_h").val("3");	}	//부산은 부산등록
		else if(ecar_loc == "7"){	$("#a_h").val("10");	}	//세종/충남/충북(대전제외)은 대구등록
...
function setEstimateInfo(response){
	...
	/*if($("#ecarFlag").val() == "3" || $("#ecarFlag").val() == "4"){*/
	if(($("#ecarFlag").val() == "3") && $("#carCompId").val() != "0056"){
...
function setViewDutyFreeOpt(){	//면세가 표기 여부가 면세가표기차량이면 리스견적불가(20190502)
