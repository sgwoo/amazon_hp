...
$(document).on("change","input[name='carOption'], input[name='car-color']",function(){
	...
	$.when(calculateDcAmt()).done(function(){
		...
		//var totalDisplayAmt2Txt = "( 차량가격 " + numberWithCommas(carAmt) + "원 + 옵션가격 " + numberWithCommas(optionAmt) + "원 + 색상가격 " + numberWithCommas(colorAmt) + "원)";
		var totalDisplayAmt2Txt = "";
		
		if (dcAmt > 0 && taxAmt > 0) {
			totalDisplayAmt2Txt = "( 차량가격 " + numberWithCommas(carAmt) + "원 + 옵션가격 " + numberWithCommas(optionAmt) + "원 + 색상가격 " + numberWithCommas(colorAmt) + "원 - " + numberWithCommas(dcAmt) + "원(제조사 D/C) - " + numberWithCommas(taxAmt) + "원(개별소비세 감면액) )";
		} else if (dcAmt > 0 && taxAmt == 0) {
			totalDisplayAmt2Txt = "( 차량가격 " + numberWithCommas(carAmt) + "원 + 옵션가격 " + numberWithCommas(optionAmt) + "원 + 색상가격 " + numberWithCommas(colorAmt) + "원 - " + numberWithCommas(dcAmt) + "원(제조사 D/C) )";
		} else if (dcAmt > 0 && taxAmt < 0) {
			totalDisplayAmt2Txt = "( 차량가격 " + numberWithCommas(carAmt) + "원 + 옵션가격 " + numberWithCommas(optionAmt) + "원 + 색상가격 " + numberWithCommas(colorAmt) + "원 - " + numberWithCommas(dcAmt) + "원(제조사 D/C) + " + numberWithCommas((-1*taxAmt)) + "원(감면 미적용 개별소비세) )";
		} else if (dcAmt <= 0 && taxAmt > 0) {
			totalDisplayAmt2Txt = "( 차량가격 " + numberWithCommas(carAmt) + "원 + 옵션가격 " + numberWithCommas(optionAmt) + "원 + 색상가격 " + numberWithCommas(colorAmt) + "원 - " + numberWithCommas(taxAmt) + "원(개별소비세 감면액) )";			
		} else if (dcAmt <= 0 && taxAmt < 0) {
			totalDisplayAmt2Txt = "( 차량가격 " + numberWithCommas(carAmt) + "원 + 옵션가격 " + numberWithCommas(optionAmt) + "원 + 색상가격 " + numberWithCommas(colorAmt) + "원 + " + numberWithCommas((-1*taxAmt)) + "원(감면 미적용 개별소비세) )";
		} else {
			totalDisplayAmt2Txt = "( 차량가격 " + numberWithCommas(carAmt) + "원 + 옵션가격 " + numberWithCommas(optionAmt) + "원 + 색상가격 " + numberWithCommas(colorAmt) + "원 )";
		}
		
		//$("#totalDisplayAmt2").text("( 차량가격 " + numberWithCommas(carAmt) + "원 + 옵션가격 " + numberWithCommas(optionAmt) + "원 + 색상가격 " + numberWithCommas(colorAmt) + "원 - 제조사DC " + numberWithCommas(dcAmt) + "원 - 개별소비세 감면액 " + numberWithCommas(taxAmt) + "원 )");
...
$(document).ready(function(){
	...
	$('#gubunSelectBox').change(function(){
		...
		//20200513 상품변경시 대여기간 및 직접입력 초기화
		var gubunValue = $("#gubunSelectBox option:selected").val();
		var valLen = $("#periodSelectBox option").size()-1;
		var minVal = Number($("#periodSelectBox option:nth-child("+valLen+")").val());
		var maxMonth = Number($("#periodSelectBox option:first").val());
		
		var period = $("#periodSelectBox option:selected").val();
		var directInputPeriod = $("#directInputPeriod").val().replace("개월", "");		
		var period_disabled_check = $("#periodSelectBox option:selected").prop("disabled");		
		
		var carUse = $("#car_use").val();
		var useMon = Number($("#use_mon").val());
		var jgCode = Number($("#jg_code").val());
		var jgCodePoint = 2000;
		...
		if ($("input[name='car-gubun']:checked").attr("id") == "car-gubun-new") {
			minVal = 12;
		}
		...
		if ($("input[name='car-gubun']:checked").attr("id") == "car-gubun-sh") {
			if (carUse == "1") {
				if (useMon < 4) {
					...
					if (gubunValue == 22 || gubunValue == 21) {
						minVal = 6;
					} else if (gubunValue == 12 || gubunValue == 11) {
						//경차
						if (jgCode < jgCodePoint) {
							minVal = 12;
						}
						//소형승용이상
						if (jgCode > jgCodePoint) {
							var min_use_mon = (60 - useMon);
							if (min_use_mon >= 12) {
								minVal = min_use_mon;
							} else {
								minVal = 12;
			...
			if (period == "directInput") {
				if (!(directInputPeriod <= maxMonth && directInputPeriod >= minVal)) {
					$("#periodSelectBox option:first").prop("selected", true);
					$("#directInputPeriod").val("");
					$("#directInputPeriod").prop("disabled", true);
				}
			} else {
				if (period_disabled_check == true) {
					$("#periodSelectBox option:first").prop("selected", true);
				}
			}			
		}
	...
	if($("#agreeDistSelectBox option:selected").val() == 'directInput'){ //직접 입력했다면
		...
		if(value == ""){
			...
			if(num_check.test(value)){
				...
				alert("약정운행거리를 정확하게 입력해주세요\n약정운행거리는 최소 10,000km 이상 최대 50,000km 이하여야 합니다.");
				return false;
			}
		}
...
//	changedAmt = Math.round(carAmt * (percent / 100));
	changedAmt = priceCutting(carAmt * (percent / 100), "R", 1000);
